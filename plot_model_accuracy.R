...
model_accuracy_scatters <- function(metric_file){
  # create an empty data frame object with necessary variables
  acc_data_all <- read.csv(metric_file)
  
  # change the names of chosen variables (extract hospital names)
  acc_data_all$hosp <- sapply(as.character(acc_data_all$hosp), function(x) unlist(strsplit(x, '_'))[1])
  ...
  # plot the metrics of model prediction 
